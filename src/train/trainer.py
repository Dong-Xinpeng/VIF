...
class QwenTrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.inf_former_lr is not None:
                lr_mapper["inf_former"] = self.args.inf_former_lr
                inf_former_parameters = [name for name, _ in opt_model.named_parameters() if "former" in name]
                

            if len(lr_mapper) > 0:
                special_lr_parameters = merger_parameters + visual_parameters + inf_former_parameters
                ...
                if inf_former_parameters:
                    optimizer_grouped_parameters.extend(
                        [
                            {
                                "params": [p for n, p in opt_model.named_parameters() if (n in decay_parameters and n in inf_former_parameters and p.requires_grad)],
                                "weight_decay": self.args.weight_decay,
                                "lr": self.args.inf_former_lr,
                            },
                            {
                                "params": [p for n, p in opt_model.named_parameters() if (n not in decay_parameters and n in inf_former_parameters and p.requires_grad)],
                                "weight_decay": 0.0,
                                "lr": self.args.inf_former_lr,
                            },
                        ]
